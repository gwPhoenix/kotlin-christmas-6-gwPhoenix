...
enum class Benefits(private val koeanText: String, private var benefitBonus: Int) {
    ...
    companion object {
        ...
        private fun getXmasDday(): Int {
            benefitInventory = 0
            if (Input.orderDate in 1..25) {
                benefitInventory = 1000 + ((Input.orderDate - 1) * 100)
            }
        ...
        private fun getWeekDay(): Int {
            benefitInventory = 0
            if (Calendar.weekendCheck() != DayOfWeek.SATURDAY
                && Calendar.weekendCheck() != DayOfWeek.FRIDAY
                && Bill.Pay.BEFORE_DISCOUNT.getPay() > 10000
            ) {
            }
